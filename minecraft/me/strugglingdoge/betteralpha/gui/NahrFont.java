...
public class NahrFont
{
    ...
    private void createFont(final Object font, final float size) {
        ...
        this.field_1308_a = Wrapper.getRenderEngine().allocateAndSetupTexture(this.bufferedImage);
    }
    ...
    private void drawer(final String text, float x, float y, final int color) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            if (text.charAt(i) == '\247' && i + 1 < text.length()) {
                ...
                final int colorCode = Wrapper.getFontRenderer().func_175064_b(oneMore);
                if (colorCode != 16777215) {
                    GL11.glColor4f((colorCode >> 16) / 255.0f, (colorCode >> 8 & 0xFF) / 255.0f, (colorCode & 0xFF) / 255.0f, alpha);
                }
                else if (oneMore == 'r') {
                    GL11.glColor4f(red, green, blue, alpha);
                }
    ...
    public void drawString(final String text, final float x, final float y, final int color, final int shadowColor) {
        this.drawString(text, x, y, FontType.SHADOW_THICK, color, shadowColor);
    }
    
    public void drawString(final String text, final float x, final float y, final FontType fontType, final int color) {
        this.drawString(text, x, y, fontType, color, (color & 0xFCFCFC) >> 2 | (color & 0xFF000000));
    }
    
    public void drawStringCentered(final String text, final float x, final float y, final FontType fontType, final int color) {
        this.drawString(text, x - this.getStringWidth(text) / 2.0f, y, fontType, color);
    }
    ...
    private void drawTexturedModalRect(final float x, final float y, final float u, final float v, final float width, final float height) {
        ...
        tessellator.addVertexWithUV(x + 0.0f, y + height, 0.0, (u + 0.0f) * 0.0039063f, (v + height) * 0.0039063f);
        tessellator.addVertexWithUV(x + width, y + height, 0.0, (u + width) * 0.0039063f, (v + height) * 0.0039063f);
        tessellator.addVertexWithUV(x + width, y + 0.0f, 0.0, (u + width) * 0.0039063f, (v + 0.0f) * 0.0039063f);
        tessellator.addVertexWithUV(x + 0.0f, y + 0.0f, 0.0, (u + 0.0f) * 0.0039063f, (v + 0.0f) * 0.0039063f);
        tessellator.draw();
    }
        ...
        while ((var2 = par0Str.indexOf(167, var2 + 1)) != -1) {
            if (var2 < var3 - 1) {
                ...
                if (this.isFormatColor(var4)) {
                    ...
                    var1 = String.valueOf(var1) + "\247" + var4;
                }
            }
        }
        return var1;
