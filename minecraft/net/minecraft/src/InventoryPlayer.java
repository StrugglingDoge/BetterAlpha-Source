    ...
    implements IInventory
{
    ...
    public void showToolTip() {
    	if(!Wrapper.getGameSettings().showToolTips)
    		return;
        if(getStackInSlot(currentItem) != null) {
        	String s = (new StringBuilder()).append("").append(StringTranslate.getInstance().translateNamedKey(getStackInSlot(currentItem).getItemName())).toString().trim();
        	if(Wrapper.getMinecraft().ingameGUI.getCurrentToolTip() != null && Wrapper.getMinecraft().ingameGUI.getCurrentToolTip().equalsIgnoreCase(s))
        		return;
        	Wrapper.getMinecraft().ingameGUI.showTooltip(s);
        }
        else
        	Wrapper.getMinecraft().ingameGUI.showTooltip("");
    }
