...
public class GuiChat extends GuiScreen
{
    ...
    public void getCommand(String message) {
    	if(message.startsWith("-")) {
			...
			for (Command command : CommandManager.getInstance().getCommands()) {
				if (("-" + command.getAlias()).toLowerCase().startsWith(message.split(" ")[0].toLowerCase())) {
					predictedCommand = command.getSyntax() + " - " + command.getDescription();
				}
			}
			
            if (!predictedCommand.equalsIgnoreCase("") && !message.contains(" ")) {
                int y = RenderUtils.newScaledResolution().getScaledHeight() - 12;
                mc.fontRenderer.drawStringWithShadow("> " + predictedCommand, 4, y, 0xFF808080);
            }
        }
    }
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        //TODO: BetterAlpha command prediction
        if(Wrapper.getGameSettings().clientCommands)
        	getCommand(message);
