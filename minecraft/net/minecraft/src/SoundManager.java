...
public class SoundManager
{
    ...
    public SoundPoolEntry getSong(String s) {
    	SoundPoolEntry soundpoolentry = soundPoolStreaming.getRandomSound();
    	if(soundpoolentry != null)
    ...
    public void playAnySound(String s, boolean musicDisc)
    {
		if (!musicDisc) {
			if (!loaded || options.musicVolume == 0.0F) {
				return;
			...
			SoundPoolEntry soundpoolentry = soundPoolMusic.getRandomSound();
			try {
				String originalName = soundpoolentry.soundName;
				String formattedName = originalName.substring(0, originalName.indexOf("."));
				if (!formattedName.equalsIgnoreCase(s)) {
					playAnySound(s, false);
					return;
				}
			} catch (StackOverflowError ex) {
				Play.failedToPlay = true;
				return;
			}
			if (soundpoolentry != null) {
				sndSystem.backgroundMusic("BgMusic", soundpoolentry.soundUrl, soundpoolentry.soundName, false);
	        ...
	        {
	            float f5 = 16F;
	            sndSystem.backgroundMusic(s1, soundpoolentry.soundUrl, soundpoolentry.soundName, false);
	            sndSystem.setVolume(s1, 0.5F * options.soundVolume);
	            sndSystem.play(s1);
	        }
		}
