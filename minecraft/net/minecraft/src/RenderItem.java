...
public class RenderItem extends Render
{
    ...
    //TODO: BetterAlpha
    public void renderItemIntoGUIBlockInfo(FontRenderer fontrenderer, RenderEngine renderengine, ItemStack itemstack,
			int i, int j, float scale) {
		if (itemstack == null)
			return;
		renderItemIntoGUIBlockInfo(fontrenderer, renderengine, itemstack.itemID, itemstack.getItemDamage(),
				itemstack.getIconIndex(), i, j, scale);
	}
	
	public void renderItemIntoGUIBlockInfo(FontRenderer fontrenderer, RenderEngine renderengine, int i, int j, int k, int l, int i1, float scale) {
        if (i < 256 && RenderBlocks.func_1219_a(Block.blocksList[i].getRenderType())) {
          int j1 = i;
          ...
          renderBlocks.renderAllFaces = field_27004_a;
          ...
          GL11.glScalef(scale,scale,scale);
          renderTexturedQuad(l / (int)scale, i1 / (int)scale, k % 16 * 16, k / 16 * 16, 16, 16);
          GL11.glEnable(2896);
        } 
        GL11.glEnable(2884);
      }
